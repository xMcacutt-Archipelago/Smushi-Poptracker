smushi_location = {}
smushi_location.__index = smushi_location
...
function smushi_location.new(name)
    local self = setmetatable({}, smushi_location)
    if name then
...
function smushi_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = smushi_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function smushi_location:connect_two_ways(exit, rule)
...
function smushi_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function smushi_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function smushi_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function smushi_location:accessibility()
...
-- 
function smushi_location:discover(accessibility, keys)
...
entry_point = smushi_location.new("entry_point")
